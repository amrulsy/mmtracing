...
router.get('/:token', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const spk = await db.queryOne<any>('SELECT * FROM work_orders WHERE id = ?', [approvalToken.woId]);
    if (spk) {
...
router.post('/:token', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (action === 'approved') {
      const spk = await db.queryOne<any>('SELECT * FROM work_orders WHERE id = ?', [approvalToken.woId]);
      if (spk) {
        const existingPembayaran = await db.queryOne('SELECT id FROM pembayaran WHERE woId = ? LIMIT 1', [spk.id]);
