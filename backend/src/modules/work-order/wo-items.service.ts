...
export class WoItemsService {
  async addItem(woId: number, input: AddWoItemInput, userId?: number) {
    ...
    if (spk.status === 'selesai' || spk.status === 'dibatalkan') {
      throw new BadRequestError(`Tidak bisa menambah item pada Work Order yang sudah ${spk.status}`);
    }
  ...
  async removeItem(woId: number, itemId: number, userId?: number) {
    const spk = await db.queryOne<any>('SELECT id, noWo, status FROM work_orders WHERE id = ?', [woId]);
    if (!spk) throw new NotFoundError('work_orders');
    if (spk.status === 'selesai' || spk.status === 'dibatalkan') {
      throw new BadRequestError(`Tidak bisa menghapus item pada Work Order yang sudah ${spk.status}`);
    }

    const item = await db.queryOne<any>('SELECT * FROM wo_items WHERE id = ? AND woId = ?', [itemId, woId]);
    if (!item) throw new NotFoundError('Item Work Order');
  ...
  async updateItem(woId: number, itemId: number, input: UpdateWoItemInput, userId?: number) {
    const spk = await db.queryOne<any>('SELECT id, noWo, status FROM work_orders WHERE id = ?', [woId]);
    if (!spk) throw new NotFoundError('work_orders');
    if (spk.status === 'selesai' || spk.status === 'dibatalkan') {
      throw new BadRequestError(`Tidak bisa mengedit item pada Work Order yang sudah ${spk.status}`);
    }
  ...
  async updateItemHpp(woId: number, itemId: number, hargaModal: number, userId?: number) {
    const spk = await db.queryOne<any>('SELECT id, noWo, status FROM work_orders WHERE id = ?', [woId]);
    if (!spk) throw new NotFoundError('work_orders');
...
export const woItemsService = new WoItemsService();
