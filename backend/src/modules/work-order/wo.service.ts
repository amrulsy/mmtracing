...
import { generateWoNo, generateInvoiceNo } from '../../shared/utils';
...
import { CreateWoInput, UpdateWoStatusInput, AddWoItemInput, UpdateWoItemInput, AddWoStageInput } from './wo.schema';
...
export class WoService {
  async findAll(query: any) {
    ...
    const [rows, totalRow] = await Promise.all([
      ...
      db.queryOne<{ c: number }>(
        `SELECT COUNT(*) AS c FROM work_orders s LEFT JOIN pelanggan p ON p.id = s.pelangganId LEFT JOIN kendaraan k ON k.id = s.kendaraanId ${where}`, params),
    ]);
  ...
  async findById(id: number) {
    const spk = await db.queryOne<EnrichedWorkOrder>('SELECT * FROM work_orders WHERE id = ?', [id]);
    ...
    const [pelanggan, kendaraan, mekanik, createdBy, items, stages, photos, pembayaranRows, garansi] = await Promise.all([
      ...
      db.query('SELECT i.*, sp.name AS spName, sp.kode AS spKode, j.name AS jName FROM wo_items i LEFT JOIN sparepart sp ON sp.id = i.sparepartId LEFT JOIN jasa j ON j.id = i.jasaId WHERE i.woId = ?', [id]),
      db.query('SELECT * FROM wo_stages WHERE woId = ? ORDER BY urutan ASC', [id]),
      db.query('SELECT * FROM wo_photos WHERE woId = ? ORDER BY createdAt DESC', [id]),
      db.query('SELECT * FROM pembayaran WHERE woId = ?', [id]),
      db.query('SELECT * FROM garansi WHERE woId = ?', [id]),
    ]);
  ...
  async create(input: CreateWoInput, userId: number) {
    ...
    const spk = await db.transaction(async (tx) => {
      ...
      await tx.insert('pembayaran', {
        noInvoice: generateInvoiceNo(), woId, totalTagihan: Math.max(0, totalHarga - diskon),
    ...
    appEventEmitter.emit('wo:created', { woId: spk.woId, noWo: spk.noWo });

    return this.findById(spk.woId);
  }

  async updateStatus(id: number, input: UpdateWoStatusInput, userId?: number) {
    const spk = await db.queryOne<any>('SELECT * FROM work_orders WHERE id = ?', [id]);
    if (!spk) throw new NotFoundError('work_orders');
    ...
    if (input.status === 'dikerjakan' && ['modifikasi', 'bubut'].includes(spk.mode)) {
      ...
      if (minDp > 0 && paid < minDp) {
        throw new BadRequestError(
          `WO ${spk.mode} memerlukan DP minimal Rp ${minDp.toLocaleString('id-ID')} sebelum mulai dikerjakan. Saat ini baru terbayar Rp ${paid.toLocaleString('id-ID')} (kurang Rp ${(minDp - paid).toLocaleString('id-ID')}).`
        );
      }
    }
    ...
    await db.transaction(async (tx) => {
      ...
      if (spk.mekanikId) {
        if (input.status === 'dikerjakan') {
          ...
          const activeSpks = await tx.queryVal<number>("SELECT COUNT(*) FROM work_orders WHERE mekanikId = ? AND status = 'dikerjakan' AND id != ?", [spk.mekanikId, id]);
          if (activeSpks === 0) {
      ...
      if (spk.status !== 'dibatalkan' && input.status === 'dibatalkan') {
        const paid = await tx.queryOne<any>('SELECT * FROM pembayaran WHERE woId = ? AND totalBayar > 0 LIMIT 1', [id]);
        if (paid) {
          throw new BadRequestError(
      ...
      if (spk.status === 'selesai' && input.status === 'dikerjakan') {
        const pembayaranAktif = await tx.queryOne<any>('SELECT * FROM pembayaran WHERE woId = ? LIMIT 1', [id]);
        if (pembayaranAktif && pembayaranAktif.status === 'lunas') {
          throw new BadRequestError('Work Order yang tagihannya sudah lunas tidak dapat ditarik kembali/rollback ke dikerjakan. (Silahkan batalkan lunas di Kasir jika perlu).');
        }
    ...
    if (input.status === 'selesai') {
      const pembayaran = await db.queryOne<{status: string, noInvoice: string}>('SELECT status, noInvoice FROM pembayaran WHERE woId = ? LIMIT 1', [id]);
      const lunas = pembayaran?.status === 'lunas';
      appEventEmitter.emit('wo:selesai', { woId: id, noWo: spk.noWo, status: input.status, isLunas: lunas, noInvoice: pembayaran?.noInvoice });
    } else if (input.status === 'kendala') {
      appEventEmitter.emit('wo:kendala', { woId: id, noWo: spk.noWo, status: input.status });
    } else if (input.status === 'dibatalkan') {
      appEventEmitter.emit('wo:dibatalkan', { woId: id, noWo: spk.noWo, status: input.status });
    } else {
      appEventEmitter.emit('wo:updated', { woId: id, noWo: spk.noWo, status: input.status });
    }
  ...
  async updateProgress(id: number, progress: number, userId?: number) {
    ...
    appEventEmitter.emit('wo:progress', { woId: id, progress });
  ...
  async delete(id: number, userId?: number) {
    ...
    if (spk.status === 'selesai') {
      throw new BadRequestError('Work Order yang sudah selesai tidak dapat dihapus');
    }
    const pembayaranRows = await db.query('SELECT id, totalBayar FROM pembayaran WHERE woId = ?', [id]);
    const hasUangMasuk = pembayaranRows.some((p: any) => Number(p.totalBayar) > 0);
    if (hasUangMasuk) {
      throw new BadRequestError('Work Order yang sudah menerima transaksi pembayaran tidak dapat dihapus!');
    }
  ...
  async restore(id: number, userId?: number) {
    const spk = await db.queryOne<any>('SELECT * FROM work_orders WHERE id = ?', [id]);
    if (!spk) throw new NotFoundError('work_orders');
    if (!spk.deletedAt) throw new BadRequestError('Work Order ini tidak dalam status terhapus');
  ...
  public async recalcTotalHarga(tx: Queryable, woId: number): Promise<number> {
    const [itemsRow, stagesRow, spkData] = await Promise.all([
      tx.queryOne<{ t: number }>('SELECT COALESCE(SUM(subtotal),0) AS t FROM wo_items WHERE woId = ?', [woId]),
      tx.queryOne<{ t: number }>('SELECT COALESCE(SUM(estimasiBiaya),0) AS t FROM wo_stages WHERE woId = ?', [woId]),
      tx.queryOne<any>('SELECT mode, diskon FROM work_orders WHERE id = ?', [woId]),
    ]);
    ...
    const pembayaran = await tx.queryOne<any>('SELECT * FROM pembayaran WHERE woId = ? LIMIT 1', [woId]);
    if (pembayaran) {
  ...
  public async recalcProgress(tx: Queryable, woId: number): Promise<number> {
    const spkData = await tx.queryOne<any>('SELECT progress, status FROM work_orders WHERE id = ?', [woId]);
    if (!spkData) return 0;
  ...
  async addPhoto(woId: number, input: { url: string; caption?: string; type?: string }) {
    const spk = await db.queryOne<any>('SELECT id, status FROM work_orders WHERE id = ?', [woId]);
    if (!spk) throw new NotFoundError('work_orders');
    if (spk.status === 'dibatalkan') throw new BadRequestError('Tidak bisa upload foto pada SPK yang sudah dibatalkan');
    const photoId = await db.insert('wo_photos', {
      woId, url: input.url, caption: input.caption || null, type: input.type || 'lampiran',
    });
    return db.queryOne('SELECT * FROM wo_photos WHERE id = ?', [photoId]);
  }

  // ── Stats ringkas untuk widget header list ────────────────────
  async stats() {
    const [antri, dikerjakan, kendala, overdue, pendingPayment] = await Promise.all([
      db.queryVal<number>("SELECT COUNT(*) FROM work_orders WHERE status = 'antri' AND deletedAt IS NULL"),
      db.queryVal<number>("SELECT COUNT(*) FROM work_orders WHERE status = 'dikerjakan' AND deletedAt IS NULL"),
      db.queryVal<number>("SELECT COUNT(*) FROM work_orders WHERE status = 'kendala' AND deletedAt IS NULL"),
      db.queryVal<number>("SELECT COUNT(*) FROM work_orders WHERE estimasiSelesai IS NOT NULL AND estimasiSelesai < NOW() AND status IN ('antri','dikerjakan','kendala') AND deletedAt IS NULL"),
  ...
  async update(id: number, input: Partial<{ keluhan: string; judulProyek: string; spesifikasi: string; prioritas: string; catatan: string; mekanikId: number | null; estimasiSelesai: string | null }>, userId?: number) {
    ...
    await db.update('work_orders', data, 'id = ?', [id]);
...
export const woService = new WoService();
