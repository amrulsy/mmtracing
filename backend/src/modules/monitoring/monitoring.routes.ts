...
import { woService } from '../work-order/wo.service';
...
router.get('/', async (_req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const [stages, photos] = spkIds.length ? await Promise.all([
      db.query('SELECT * FROM wo_stages WHERE woId IN (?) ORDER BY urutan ASC', [spkIds]),
      db.query('SELECT * FROM wo_photos WHERE woId IN (?) ORDER BY createdAt DESC', [spkIds]),
    ]) : [[], []];
    const stageMap = new Map<number, any[]>();
    for (const st of stages) { if (!stageMap.has(st.woId)) stageMap.set(st.woId, []); stageMap.get(st.woId)!.push(st); }
    const photoMap = new Map<number, any[]>();
    for (const ph of photos) { if (!photoMap.has(ph.woId)) photoMap.set(ph.woId, []); photoMap.get(ph.woId)!.push(ph); }
...
router.get('/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const data = await db.queryOne<any>('SELECT * FROM work_orders WHERE id = ?', [id]);
    ...
    const [pelanggan, kendaraan, mekanik, items, stages, photos] = await Promise.all([
      ...
      db.query('SELECT * FROM wo_items WHERE woId = ?', [id]),
...
router.put('/:id/status', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const data = await woService.updateStatus(
...
router.post('/:id/foto', upload.single('photo'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const photo = await db.queryOne('SELECT * FROM wo_photos WHERE id = ?', [photoId]);
