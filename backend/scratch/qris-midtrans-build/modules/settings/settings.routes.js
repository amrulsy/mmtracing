...
router.put('/qris', auth_1.authMiddleware, (0, auth_1.requirePermission)('settings', 'full'), (0, validate_1.validate)(zod_1.z.object({
    payload: zod_1.z.string().max(4096).default(''), enabled: zod_1.z.boolean(), mode: zod_1.z.enum(['static', 'midtrans', 'legacy']).default('legacy'),
})), async (req, res, next) => {
    try {
        (0, utils_1.sendSuccess)(res, await (0, qris_service_1.saveQrisSettings)(req.body.payload, req.body.enabled, req.user.id, req.body.mode), 'Pengaturan QRIS disimpan');
    }
    catch (e) {
        next(e);
    }
