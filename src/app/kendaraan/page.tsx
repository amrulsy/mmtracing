...
export default function KendaraanPage() {
  ...
  const handleDeletePelanggan = async () => {
    if (!active) return;
    if (!confirm(`Yakin ingin menghapus pelanggan "${active.name}"? Tindakan ini tidak bisa dibatalkan.`)) return;
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex flex-col gap-1">
          <h1 className="text-3xl font-bold tracking-tight">Pelanggan & Kendaraan</h1>
          <p className="text-muted-foreground">Kelola database pelanggan — servis kendaraan maupun jasa bubut lepas.</p>
        </div>
        <div className="flex gap-2">
          <Link href="/kendaraan/tambah" className="flex items-center gap-2 text-sm bg-surface border border-surface-border px-3 py-2 rounded-xl hover:bg-surface-hover font-medium">
            <Car size={16} /> + Pelanggan
          </Link>
          <Link href="/spk/bubut" className="flex items-center gap-2 btn-glossy bg-primary text-primary-foreground px-3 py-2 rounded-xl font-medium shadow-glossy-primary hover:shadow-glossy-primary-dark text-sm">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        {/* Sidebar List */}
        <div className="glass-panel lg:col-span-1 h-[calc(100vh-12rem)] flex flex-col overflow-hidden">
          ...
          {!active ? (
            ...
            <>
              ...
              <div className="glass-panel p-6 relative overflow-hidden">
                ...
                <div className="mt-6 grid grid-cols-2 md:grid-cols-3 gap-4 pt-6 border-t border-surface-border">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  {(active.kendaraan || []).map((k) => (
                    <div key={k.id} className="border border-surface-border rounded-xl p-4 bg-surface-hover/30 hover:bg-surface-hover/60 transition-colors">
                      ...
                      <div className="flex gap-2">
                        <Link href={`/spk/create?pelangganId=${active.id}&kendaraanId=${k.id}`} className="flex-1 text-center text-xs bg-primary/10 text-primary hover:bg-primary/20 py-1.5 rounded-lg transition-colors border border-primary/20 font-medium">Buat SPK</Link>
                        <Link href={`/kendaraan/${k.id}`} className="flex-1 text-xs bg-surface border border-surface-border hover:bg-surface-hover py-1.5 rounded-lg transition-colors flex items-center justify-center gap-1 font-medium">
