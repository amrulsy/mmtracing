...
export default function SparepartPage() {
    ...
    toast.confirm(
      ...
      async () => {
        ...
        try {
          ...
          toast.error("Gagal", err instanceof Error ? err.message : "Gagal menghapus sparepart");
  ...
  useEffect(() => {
    setPage(1);
  }, [searchDebounced, kategoriFilter, lowStockOnly]);

  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      if (lowStockOnly) {
        const res = await api.get<Sparepart[]>("/sparepart/low-stock");
        setParts(res.data || []);
        setPagination(null);
      } else {
  ...
  }, [page, searchDebounced, kategoriFilter, lowStockOnly]);
  ...
  return (
    <div className="space-y-4 lg:space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <Link href="/app/master/sparepart/tambah" className="flex items-center justify-center gap-1.5 btn-glossy bg-primary text-primary-foreground px-3 py-2 rounded-xl text-sm font-medium shadow-glossy-primary hover:shadow-glossy-primary-dark">
      ...
      <div className="glass-panel p-3 flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => setLowStockOnly(v => !v)} className={`flex items-center justify-center gap-1.5 text-xs px-3 py-2 rounded-lg font-medium border transition-colors ${lowStockOnly ? "bg-amber-500/10 border-amber-500/40 text-amber-600" : "bg-surface border-surface-border hover:bg-surface-hover"}`}><Filter size={14} /> {lowStockOnly ? "Menipis ✓" : "Stok Menipis"}</button>
        </div>
      </div>
                      ...
                      return (
                        <tr key={p.id} className="bg-surface border-b border-surface-border hover:bg-surface-hover/30 transition-colors">
                          ...
                          <td className="px-4 py-3">
                            <Link href={`/app/master/sparepart/${p.id}`} className="font-medium hover:text-primary">{p.name}</Link>
                          ...
                          <td className="px-4 py-3 text-right">
                            <div className="flex justify-end gap-1">
                              <Link href={`/app/master/sparepart/${p.id}`} className="p-1.5 rounded-lg hover:bg-surface-hover text-muted-foreground hover:text-foreground"><Edit size={14} /></Link>
              ...
              return (
                <div key={p.id} className="glass-panel p-3 flex items-center gap-3">
                  ...
                  <div className="flex flex-col gap-1 shrink-0">
                    <Link href={`/app/master/sparepart/${p.id}`} className="p-2 rounded-lg hover:bg-surface-hover text-muted-foreground hover:text-foreground"><Edit size={15} /></Link>
                    <button onClick={() => handleDelete(p.id, p.name)} disabled={deleting === p.id}
