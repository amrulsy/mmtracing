...
export default function DashboardPage() {
 ...
 <kpiCard.icon size={20} />
 </div>
 <div className="min-w-0 flex-1">
 <p className="text-[9px] lg:text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-0.5 truncate">{kpiCard.label}</p>
 <p className={`text-xl lg:text-2xl font-black font-mono leading-none ${kpiCard.alert ? "text-red-500" : ""}`}>{kpiCard.value}</p>
 {kpiCard.sub && <p className="text-[10px] text-muted-foreground mt-1 truncate">{kpiCard.sub}</p>}
 </div>
 </div>
 ))}
 </div>
 ...
 <Link key={i} href={a.href} className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold whitespace-nowrap shrink-0 transition-all active:scale-95 ${a.primary ? "bg-red-600 text-white shadow-none" : "bg-surface border border-surface-border text-muted-foreground hover:text-foreground"}`}>
 ...
 <Link key={i} href={a.href} className={`flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-bold transition-all active:scale-[0.98] ${a.primary ? "bg-red-600 text-white hover:bg-red-700" : "bg-surface border border-surface-border text-muted-foreground hover:text-foreground hover:bg-surface-hover"}`}>
