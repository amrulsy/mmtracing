...
export default function InventarisPage() {
 ...
 const [logsMasuk, setLogsMasuk] = useState<InventarisLog[]>([]);
 const [logsKeluar, setLogsKeluar] = useState<InventarisLog[]>([]);
 ...
 <Link href="/app/inventaris/masuk" className="flex items-center gap-2 text-sm bg-primary text-primary-foreground px-3 py-2 rounded-xl hover:opacity-90 font-medium ">
 ...
 {[
 { label: "Total Item", value: String(summary.totalItem), icon: Package, color: "" },
 { label: "Nilai Stok", value: formatRp(summary.nilaiStok), icon: TrendingUp, color: "text-primary" },
 ...
 <div key={i} className="glass-panel p-3 lg:p-4 flex items-start gap-2 lg:gap-3">
 <div className={`w-8 h-8 lg:w-10 lg:h-10 rounded-xl bg-surface-hover flex items-center justify-center shrink-0 ${s.color}`}>
 <s.icon size={18} />
 </div>
 <div className="min-w-0">
 <p className="text-[10px] lg:text-xs text-muted-foreground font-medium">{s.label}</p>
 <p className={`text-lg lg:text-xl font-bold mt-0.5 truncate ${s.color}`}>{s.value}</p>
 </div>
 </div>
 ))}
 </div>
 ...
 <span className="text-sm font-bold font-mono text-emerald-600">{formatRp(Number(log.totalHarga))}</span>
 </div>
 ))
 )}
 </div>
 ...
 <span className="text-sm font-bold font-mono text-red-500">{formatRp(Number(log.totalHarga))}</span>
 </div>
 ))
 )}
 </div>
