...
export default function SettingsLayout({ children }: { children: React.ReactNode }) {
 ...
 return (
 <Link key={i} href={tab.href} className={`flex items-center gap-2 lg:gap-3 px-3 py-2 lg:py-2.5 rounded-xl text-xs lg:text-sm font-medium whitespace-nowrap transition-colors ${isActive ? "bg-primary text-primary-foreground " : "text-muted-foreground hover:text-foreground hover:bg-surface-hover"}`}>
 <tab.icon size={16} />
 <span>{tab.label}</span>
 </Link>
 );
