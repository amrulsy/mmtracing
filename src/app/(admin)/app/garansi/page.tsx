    ...
    <div className="space-y-4 lg:space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out overflow-hidden">
      <div className="flex flex-col gap-1">
        <h1 className="text-2xl lg:text-3xl font-bold tracking-tight flex items-center gap-2 lg:gap-3">
          <Shield className="text-primary" size={24} /> Manajemen Garansi
        </h1>
        <p className="text-muted-foreground text-sm">Track garansi jasa & sparepart. Alert otomatis jika hampir habis.</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 lg:gap-4">
        {[
          ...
          <div key={i} className="glass-panel p-3 lg:p-4 flex items-center gap-2 lg:gap-3">
            <div className={`w-8 h-8 lg:w-10 lg:h-10 rounded-xl bg-surface-hover flex items-center justify-center shrink-0 ${s.color}`}><s.icon size={18} /></div>
            <div>
              <p className={`text-xl lg:text-2xl font-bold ${s.color}`}>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {filteredWarranties.length === 0 ? (
            ...
            return (
              <div key={w.id} className={`glass-panel p-3 lg:p-4 hover:shadow-sm transition-shadow ${w.computedStatus === "hampir" ? "border-l-4 border-l-amber-500" : w.computedStatus === "expired" ? "opacity-50" : ""}`}>
                <div className="flex items-center gap-3">
                  <div className={`w-8 h-8 lg:w-10 lg:h-10 rounded-xl flex items-center justify-center shrink-0 ${w.computedStatus === "expired" ? "bg-red-500/10" : w.computedStatus === "hampir" ? "bg-amber-500/10" : "bg-emerald-500/10"}`}>
                    {w.computedStatus === "expired" ? <XCircle size={18} className="text-red-500" /> : w.computedStatus === "hampir" ? <AlertTriangle size={18} className="text-amber-500" /> : <Shield size={18} className="text-emerald-500" />}
                  </div>
                  ...
                  <div className="text-right shrink-0">
                    {w.computedStatus === "expired" ? (
                      ...
                      <p className={`text-sm font-bold ${w.daysLeft <= 7 ? "text-amber-500" : "text-emerald-600"}`}>{w.daysLeft}d</p>
                    )}
                    <p className="text-[10px] text-muted-foreground">sisa</p>
                  </div>
                </div>
                <p className="text-[10px] text-muted-foreground mt-1 pl-11 lg:pl-[52px]">{formatDate(w.startDate)} → {formatDate(w.endDate)}</p>
              </div>
            );
          })}
        </div>
        ...
        <div className="space-y-3">
          {claims.length === 0 ? (
            ...
            <div key={c.id} className={`glass-panel p-3 sm:p-4 lg:p-5 ${c.status !== "selesai" ? "border-l-4 border-l-primary" : ""}`}>
