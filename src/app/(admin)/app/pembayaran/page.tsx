...
export default function PembayaranPage() {
  ...
  <>
    <div className="glass-panel p-3 lg:p-4 flex items-center gap-3 relative overflow-hidden">
      ...
      <div className="min-w-0 flex-1">
        <p className="text-[9px] lg:text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-0.5 truncate">Menunggu</p>
        <p className="text-xl lg:text-2xl font-black font-mono leading-none text-primary">{formatRp(summary.menunggu.total)}</p>
        <p className="text-[10px] text-muted-foreground mt-1 truncate">{summary.menunggu.count} Invoice</p>
      </div>
    </div>
    <div className="glass-panel p-3 lg:p-4 flex items-center gap-3 relative overflow-hidden">
      ...
      <div className="min-w-0 flex-1">
        <p className="text-[9px] lg:text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-0.5 truncate">Hari Ini</p>
        <p className="text-xl lg:text-2xl font-black font-mono leading-none text-emerald-500">{formatRp(summary.hariIni.total)}</p>
        <p className="text-[10px] text-muted-foreground mt-1 truncate">{summary.hariIni.count} Transaksi</p>
      </div>
    </div>
    <div className="glass-panel p-3 lg:p-4 flex items-center gap-3 relative overflow-hidden">
      ...
      <div className="min-w-0 flex-1">
        <p className="text-[9px] lg:text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-0.5 truncate">Bulan Ini</p>
        <p className="text-xl lg:text-2xl font-black font-mono leading-none text-amber-500">{formatRp(summary.bulanIni)}</p>
        <p className="text-[10px] text-muted-foreground mt-1 truncate">Total pendapatan</p>
      </div>
    </div>
  </>
 ) : (
 ...
 {item.status !== "lunas" && (
 <Link href={`/app/pembayaran/kasir?id=${item.id}`} className="px-3 py-1.5 text-xs font-medium rounded-lg bg-primary text-white shadow-sm hover:">Bayar</Link>
 )}
 <Link href={`/app/pembayaran/${item.id}`} className="p-1.5 text-muted-foreground hover:text-foreground border border-surface-border rounded-lg inline-block" title="Lihat Detail"><Eye size={16} /></Link>
 </td>
 </tr>
 );
 })
 ...
 invoices.map((item) => (
 <Link key={item.id} href={`/app/pembayaran/${item.id}`} className="glass-panel p-4 space-y-2 block">
 ...
 <p className="text-sm font-bold text-primary font-mono">{formatRp(item.sisaBayar ?? 0)}</p>
 </div>
 )}
 </div>
 </Link>
