...
export default function KasirPage() {
 ...
 className={`flex-1 py-2.5 rounded-xl text-sm font-medium transition-all ${mobileTab === 'antrian' ? 'bg-primary text-white ' : 'bg-surface border border-surface-border'}`}
 >
 Antrian ({filteredInvoices.length})
 </button>
 <button
 onClick={() => setMobileTab('bayar')}
 className={`flex-1 py-2.5 rounded-xl text-sm font-medium transition-all ${mobileTab === 'bayar' ? 'bg-primary text-white ' : 'bg-surface border border-surface-border'}`}
 >
 ...
 <button onClick={handleBayarPrep} disabled={processing || numBayar <= 0} className="w-full mt-6 bg-primary text-primary-foreground py-4 rounded-xl font-bold text-lg hover: transition-all flex items-center justify-center gap-2 disabled:opacity-50">
