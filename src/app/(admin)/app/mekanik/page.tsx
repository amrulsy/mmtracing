...
export default function MekanikPage() {
 ...
 .catch((err: unknown) => setError(err instanceof Error ? err.message : "Gagal memuat data mekanik"))
 ...
 <h1 className="text-2xl lg:text-3xl font-bold tracking-tight">Tim Mekanik</h1>
 <p className="text-muted-foreground text-sm">Kelola data mekanik, spesialisasi, dan jadwal ketersediaan.</p>
 </div>
 <button
 onClick={() => setShowModal(true)}
 className="flex items-center gap-2 bg-primary text-primary-foreground px-4 py-2 rounded-xl font-medium hover:"
 >
 ...
 return (
 <Link key={m.id} href={`/app/mekanik/${m.id}`} className="glass-panel p-5 hover:-translate-y-1 hover:shadow-glossy transition-all cursor-pointer group">
 ...
 <button type="submit" disabled={submitting || !form.name.trim()} className="w-full mt-2 py-2.5 bg-primary text-white font-bold rounded-xl disabled:opacity-50 flex justify-center items-center gap-2 ">
