...
export default function SpkAnalyticsPage() {
 ...
 api.get<AnalyticsData>("/work-order/analytics", params)
 ...
 <Link href="/app/work-order" className="p-2 hover:bg-surface-hover rounded-xl">
 <ArrowLeft size={18} />
 </Link>
 <div className="flex-1">
 <h1 className="text-xl font-bold flex items-center gap-2"><TrendingUp size={20} className="text-primary" /> Analitik Work Order</h1>
 ...
 <Stat label="Total Work Order" value={totalCount.toLocaleString("id-ID")} icon={<Wrench size={16} />} />
 ...
 <Stat label="Avg/WO Selesai" value={formatRupiah(totalCount > 0 ? Math.round(totalOmzet / Math.max(1, totalCount)) : 0)} icon={<Users size={16} />} />
 </div>
 ...
 <p className="text-xs text-muted-foreground">Work Order total</p>
 ...
 {data.trend.map(t => (
 <div key={t.month} className="flex flex-col items-center justify-end gap-1 h-full" title={`${fmtMonth(t.month)}: ${t.total} WorkOrder · ${formatRupiah(t.revenue)}`}>
 ...
 <p className="text-sm text-muted-foreground italic text-center py-6">Belum ada WO selesai oleh mekanik</p>
 ) : (
 <div className="divide-y divide-surface-border">
 ...
 <p className="text-[11px] text-muted-foreground">{m.spkSelesai} WO selesai</p>
 </div>
