...
import type { WorkOrder, Mekanik } from "@/lib/types";

export function useWoDetail(id: string) {
 const router = useRouter();

 const [spk, setSpk] = useState<WorkOrder | null>(null);
 ...
 try {
 setError("");
 const res = await api.get<WorkOrder>(`/work-order/${id}`);
 setSpk(res.data);
 ...
 const res = await api.put<WorkOrder>(`/work-order/${id}/status`, body);
 setSpk(res.data);
 setProgress(res.data.progress ?? 0);
 ...
 setEditSaving(true);
 try {
 const res = await api.put<WorkOrder>(`/work-order/${id}`, {
 ...
 try {
 const res = await api.put<WorkOrder>(`/work-order/${id}/mekanik`, {
 ...
 router.push(`/app/work-order/create?mode=${spk.mode}&clone=1`);
 };
 ...
 setSendingWA(kind);
 try {
 await api.post(`/work-order/${id}/whatsapp`, { kind });
 ...
 try {
 const res = await api.put<WorkOrder>(`/work-order/${id}/progress`, { progress });
 setSpk(res.data);
 ...
 try {
 const res = await api.patch<WorkOrder>(`/work-order/${id}/items/${itemId}`, { status: next });
 setSpk(res.data);
 setProgress(res.data.progress ?? 0);
 } catch (err: unknown) {
 toast.error("Gagal", err instanceof Error ? err.message : "Terjadi kesalahan");
 ...
 try {
 const res = await api.patch<WorkOrder>(`/work-order/${id}/stages/${stageId}`, { status: next });
 setSpk(res.data);
 setProgress(res.data.progress ?? 0);
 } catch (err: unknown) {
 toast.error("Gagal", err instanceof Error ? err.message : "Terjadi kesalahan");
 ...
 await api.upload(`/work-order/${id}/photos`, fd);
 ...
 // ── Delete ──────────────────────────────────────────────────────
 const handleDelete = () => {
 toast.confirm(`Yakin ingin mengHapus Work Order ${spk?.noWo}?`, async () => {
 try {
 await api.delete(`/work-order/${id}`);
 toast.success("spk dihapus");
 router.push("/app/work-order");
 } catch (err: unknown) {
 ...
 return {
  // Data
  spk, workOrder: spk, WorkOrder: spk, loading, error, fetchSpk,
 // Progress
