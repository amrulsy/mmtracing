...
export function WoHeader({ spk, id, isAdmin, openEdit, handleClone, handleSendWA, sendingWA, handleDelete }: SpkHeaderProps) {
 return (
 <div className="flex flex-wrap items-start gap-3">
 <Link href="/app/work-order" className="p-2 hover:bg-surface-hover rounded-xl border border-surface-border glass transition-colors shrink-0">
 <ArrowLeft size={20} />
 </Link>
 <div className="flex-1 min-w-0">
 <div className="flex flex-wrap items-center gap-2">
 <h1 className="text-xl lg:text-2xl font-bold tracking-tight font-mono">{spk.noWo}</h1>
 ...
 onClick={handleClone}
 className="flex items-center gap-1.5 px-3 py-2 text-xs font-bold border border-surface-border rounded-xl hover:bg-surface-hover transition-colors"
 aria-label="Clone Work Order"
 title="Buat Work Order baru berdasarkan data Work Order ini"
 >
 <Copy size={14} /> Clone
 </button>
 {spk.pelanggan?.phone && (
 ...
 <button onClick={() => handleSendWA("created")} className="w-full text-left text-xs px-3 py-2 rounded-lg hover:bg-surface-hover">WO Dibuat (invoice)</button>
 ...
 <button onClick={handleDelete} className="flex items-center gap-2 px-3 py-2 text-sm font-medium border border-red-500/30 text-red-500 rounded-xl hover:bg-red-500/10 transition-colors" aria-label="Hapus Work Order">
 <Trash2 size={15} />
 </button>
 )}
 </div>
