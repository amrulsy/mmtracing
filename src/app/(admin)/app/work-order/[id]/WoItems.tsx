...
import type { WorkOrder, Sparepart, Jasa } from "@/lib/types";
...
export function WoItems({ spk, id, fetchSpk, togglingId, handleToggleItem }: SpkItemsProps) {
 ...
 await api.post(`/work-order/${id}/items`, payload);
 await fetchSpk();
 ...
 try {
 await api.delete(`/work-order/${id}/items/${itemId}`);
 await fetchSpk();
