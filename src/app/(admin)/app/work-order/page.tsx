...
import type { WorkOrder, Pagination, Mekanik } from "@/lib/types";
...
export default function SpkPage() {
 ...
 const [spks, setSpks] = useState<WorkOrder[]>([]);
 ...
 api.get<SpkStats>("/work-order/stats").then(r => setStats(r.data)).catch(() => { /* silent */ });
 }, []);
 ...
 const res = await api.getPaginated<WorkOrder>("/work-order", params);
 setSpks(res.data);
 setPagination(res.pagination);
 } catch (err: unknown) {
 setError(err instanceof Error ? err.message : "Gagal memuat data Work Order");
 } finally {
 setLoading(false);
 }
 ...
 if (!WorkOrder.estimasiSelesai) return false;
 if (WorkOrder.status === "selesai" || WorkOrder.status === "dibatalkan") return false;
 return new Date(WorkOrder.estimasiSelesai) < new Date();
 };

 const paymentStatus = (WorkOrder: WorkOrder): { label: string; color: string } | null => {
 const pem = WorkOrder.pembayaran?.[0];
 if (!pem) return null;
 ...
 <h1 className="text-2xl lg:text-3xl font-bold tracking-tight">Manajemen Work Order</h1>
 <p className="text-muted-foreground text-sm">Kelola Work Order untuk Servis maupun Modifikasi.</p>
 </div>
 <div className="flex gap-2 flex-wrap">
 <Link href="/app/work-order/analytics" className="flex items-center gap-1.5 text-sm bg-surface border border-surface-border px-3 py-2 rounded-xl font-medium hover:bg-surface-hover">
 <TrendingUp size={16} /> Analitik
 </Link>
 <Link href="/app/work-order/create?mode=bubut" className="flex items-center gap-1.5 text-sm bg-surface border border-surface-border px-3 py-2 rounded-xl font-medium hover:bg-surface-hover">
 <Hammer size={16} /> Bubut Lepas
 </Link>
 <Link href="/app/work-order/create" className="flex items-center gap-1.5 bg-primary text-primary-foreground px-3 py-2 rounded-xl text-sm font-medium hover:">
 <Plus size={16} /> WorkOrder Baru
 </Link>
 </div>
 </div>
 ...
 <button key={t} onClick={() => setTab(t)} className={`px-3 py-1.5 text-xs font-medium rounded-lg whitespace-nowrap transition-colors ${tab === t ? "bg-primary text-white " : "text-muted-foreground hover:bg-surface-hover border border-surface-border"}`}>
 ...
 placeholder="Cari No WO, pelanggan, kendaraan... (tekan /)"
 ...
 <th className="px-6 py-4 font-semibold">No. WO / Tanggal</th>
 ...
 {searchDebounced ? `Tidak ada hasil untuk "${searchDebounced}"` : "Belum ada data Work Order"}
 </td>
 </tr>
 ) : (
 spks.map((WorkOrder) => {
 const overdue = isOverdue(WorkOrder);
 const payment = paymentStatus(WorkOrder);
 return (
 <tr key={WorkOrder.id} className={`bg-surface border-b border-surface-border hover:bg-surface-hover/50 transition-colors ${overdue ? "border-l-4 border-l-red-500" : ""}`}>
 <td className="px-6 py-4">
 <span className={`px-2.5 py-1 rounded-full text-xs font-medium border ${statusStyle(WorkOrder.status)}`}>{WorkOrder.status}</span>
 {overdue && (
 ...
 {prioritasIcon(WorkOrder.prioritas)}
 {WorkOrder.noWo}
 </div>
 <div className="text-xs text-muted-foreground">{formatTanggal(WorkOrder.createdAt)}</div>
 ...
 <Calendar size={10} /> ETA {formatTanggal(WorkOrder.estimasiSelesai)}
 </div>
 )}
 </td>
 <td className="px-6 py-4">
 <div className="font-medium">{WorkOrder.pelanggan?.name || "—"}</div>
 <div className="text-xs text-muted-foreground">
 {WorkOrder.kendaraan ? `${WorkOrder.kendaraan.name} (${WorkOrder.kendaraan.plat})` : "—"}
 </div>
 </td>
 <td className="px-6 py-4">
 {WorkOrder.mekanik ? (
 <div className="text-xs font-medium">{WorkOrder.mekanik.name}</div>
 ) : (
 <div className="text-xs text-red-500 italic">Belum diassign</div>
 )}
 <div className="inline-flex items-center gap-1 text-[10px] text-muted-foreground mt-0.5 capitalize"><FileText size={10} />{WorkOrder.mode}</div>
 </td>
 <td className="px-6 py-4">
 {(WorkOrder.status === "dikerjakan" || WorkOrder.status === "selesai" || WorkOrder.status === "kendala") && (
 <div className="w-24">
 <div className="flex justify-between text-[10px] text-muted-foreground mb-0.5">
 <span>{WorkOrder.status === "selesai" ? 100 : WorkOrder.progress ?? 0}%</span>
 </div>
 <div className="h-1 bg-surface-border rounded-full overflow-hidden">
 <div className={`h-full rounded-full transition-all ${WorkOrder.status === "selesai" ? "bg-emerald-500" : WorkOrder.status === "kendala" ? "bg-red-500" : "bg-blue-500"}`} style={{ width: `${WorkOrder.status === "selesai" ? 100 : WorkOrder.progress ?? 0}%` }} />
 </div>
 </div>
 )}
 </td>
 <td className="px-6 py-4">
 <p className="font-bold text-sm">{formatRupiah(Number(WorkOrder.totalTagihan ?? (Number(WorkOrder.totalHarga) - Number(WorkOrder.diskon ?? 0))), "compact")}</p>
 {payment && <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${payment.color}`}>{payment.label}</span>}
 </td>
 <td className="px-6 py-4 text-right">
 <Link href={`/app/work-order/${WorkOrder.id}`} className="text-primary hover:text-primary-hover font-medium text-sm">Detail</Link>
 </td>
 </tr>
 );
 })
 ...
 const payment = paymentStatus(WorkOrder);
 return (
 <Link key={WorkOrder.id} href={`/app/work-order/${WorkOrder.id}`} className={`glass-panel p-4 flex items-center gap-3 active:scale-[0.98] transition-transform block ${overdue ? "border-l-4 border-l-red-500" : ""}`}>
 <div className="flex-1 min-w-0">
 <div className="flex items-center gap-2 mb-1 flex-wrap">
 <span className="text-xs font-mono text-muted-foreground">{WorkOrder.noWo}</span>
 <span className={`text-[10px] font-bold uppercase px-1.5 py-0.5 rounded-full border ${statusStyle(WorkOrder.status)}`}>{WorkOrder.status}</span>
 {overdue && <span className="text-[9px] font-bold text-red-600 uppercase">Overdue</span>}
 </div>
 <div className="flex items-center gap-1.5">
 {prioritasIcon(WorkOrder.prioritas)}
 <p className="font-semibold text-sm truncate">{WorkOrder.pelanggan?.name || "—"}</p>
 </div>
 <p className="text-xs text-muted-foreground truncate">
 {WorkOrder.kendaraan ? `${WorkOrder.kendaraan.name}` : "—"} • {WorkOrder.mode} {WorkOrder.mekanik && `• ${WorkOrder.mekanik.name}`}
 </p>
 <div className="flex items-center gap-2 mt-1">
 <p className="text-sm font-bold text-primary">{formatRupiah(Number(WorkOrder.totalTagihan ?? (Number(WorkOrder.totalHarga) - Number(WorkOrder.diskon ?? 0))), "compact")}</p>
 ...
 {((page - 1) * pagination.limit) + 1}-{Math.min(page * pagination.limit, pagination.total)} dari {pagination.total} WorkOrder
 </span>
...
function StatCard({ label, value, icon, onClick, active, className, highlight }: { label: string; value: number; icon: React.ReactNode; onClick: () => void; active: boolean; className: string; highlight?: boolean }) {
 ...
 type="button"
 onClick={onClick}
 className={`glass-panel p-3 flex items-center gap-3 text-left border transition-all hover:scale-[1.02] ${className} ${active ? "ring-2 ring-current" : ""} ${highlight && value > 0 ? "animate-pulse" : ""}`}
 aria-pressed={active}
 >
 <div className="w-8 h-8 rounded-lg bg-white/20 dark:bg-black/20 flex items-center justify-center shrink-0">
   {icon}
 </div>
 <div className="min-w-0 flex-1">
   <span className="text-[9px] font-bold uppercase tracking-wider block mb-0.5 opacity-80">{label}</span>
