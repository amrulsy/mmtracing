...
import { Calendar, Loader2, Wrench, Package, Users, AlertTriangle, TrendingUp, TrendingDown, DollarSign, PieChart, RefreshCw, ChevronDown, Download, ArrowUp, ArrowDown, Minus, Scissors, Wallet, Receipt, Repeat, CreditCard, Banknote } from "lucide-react";
...
export default function LaporanPage() {
 ...
 className="flex items-center gap-2 bg-surface text-foreground border border-surface-border px-4 py-2 rounded-xl font-bold hover:bg-surface-hover"
 >
 <Calendar size={18} />
 ...
 disabled={loading}
 title="Refresh data"
 className="flex items-center justify-center bg-surface text-foreground border border-surface-border px-3 py-2 rounded-xl hover:bg-surface-hover disabled:opacity-50"
 >
 ...
 className="flex items-center gap-1.5 bg-red-600 text-white px-3 py-2 rounded-xl text-xs font-bold hover:bg-red-700"
 >
 ...
 <div className="glass-panel p-3 lg:p-4 flex items-center gap-3 border-emerald-500/20 bg-background relative overflow-hidden">
 <div className="w-10 h-10 rounded-xl bg-emerald-500/10 text-emerald-500 flex items-center justify-center shrink-0">
 <TrendingUp size={20} />
 </div>
 <div className="min-w-0 flex-1">
 <span className="text-[9px] lg:text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-0.5 truncate block">Omzet</span>
 <div className="text-xl lg:text-2xl font-black font-mono leading-none">
 {loading ? <Skeleton className="h-6 w-24" /> : formatRp(labaRugi?.pendapatan || 0)}
 </div>
 ...
 <div
 className="glass-panel p-3 lg:p-4 flex items-center gap-3 border-orange-500/20 bg-background relative overflow-hidden"
 ...
 <div className="min-w-0 flex-1">
 <span className="text-[9px] lg:text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-0.5 truncate block">HPP (Modal)</span>
 <div className="text-xl lg:text-2xl font-black font-mono leading-none">
 {loading ? <Skeleton className="h-6 w-24" /> : formatRp(labaRugi?.hpp || 0)}
 </div>
 {labaRugi && <p className="text-[10px] text-muted-foreground mt-1 truncate">Part {formatRp(labaRugi.hppSparepart)} · Jasa {formatRp(labaRugi.hppJasa)}</p>}
 </div>
 </div>
 ...
 <div className="min-w-0 flex-1">
 <span className="text-[9px] lg:text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-0.5 truncate block">Laba Kotor</span>
 <div className="text-xl lg:text-2xl font-black font-mono leading-none">
 {loading ? <Skeleton className="h-6 w-24" /> : formatRp(labaRugi?.labaKotor || 0)}
 </div>
 <p className="text-[10px] text-muted-foreground mt-1 truncate">Margin: {loading ? "..." : ((labaRugi?.labaKotor || 0) / (labaRugi?.pendapatan || 1) * 100).toFixed(1)}%</p>
 </div>
 </div>

 <div className="glass-panel p-3 lg:p-4 flex items-center gap-3 border-red-500/20 bg-background relative overflow-hidden">
 <div className="w-10 h-10 rounded-xl bg-red-500/10 text-red-500 flex items-center justify-center shrink-0">
 <CreditCard size={20} />
 </div>
 <div className="min-w-0 flex-1">
 <span className="text-[9px] lg:text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-0.5 truncate block">Pengeluaran</span>
 <div className="text-xl lg:text-2xl font-black font-mono leading-none">
 {loading ? <Skeleton className="h-6 w-24" /> : formatRp(labaRugi?.pengeluaran || 0)}
 </div>
 {kpi && <div className="mt-1 flex items-center gap-1"><DeltaBadge pct={kpi.change.pengeluaran} invert /> <span className="text-[10px] text-muted-foreground">vs periode lalu</span></div>}
 </div>
 </div>

 <div className="glass-panel col-span-2 sm:col-span-4 lg:col-span-1 p-3 lg:p-4 flex items-center gap-3 border-indigo-500/30 bg-background ring-1 relative overflow-hidden">
 <div className="w-10 h-10 rounded-xl bg-indigo-500/20 text-indigo-400 flex items-center justify-center shrink-0">
 <Banknote size={20} />
 </div>
 <div className="min-w-0 flex-1">
 <span className="text-[9px] lg:text-[10px] font-bold text-indigo-400 uppercase tracking-widest mb-0.5 truncate block">Laba Bersih</span>
 <div className={`text-xl lg:text-2xl font-black font-mono leading-none ${(labaRugi?.labaBersih || 0) < 0 ? "text-red-500" : "text-emerald-500"}`}>
 {loading ? <Skeleton className="h-6 w-24" /> : formatRp(labaRugi?.labaBersih || 0)}
 </div>
 {kpi && <div className="mt-1 flex items-center gap-1"><DeltaBadge pct={kpi.change.labaBersih} /> <span className="text-[10px] text-muted-foreground">vs periode lalu</span></div>}
 </div>
 </div>
 </div>
 ...
 {/* Mini: Basis info */}
 <div className="glass-panel p-4 lg:p-6 flex flex-col gap-3 bg-background">
