...
export default function InspeksiDetailPage({ params }: { params: Promise<{ id: string }> }) {
 ...
 if (lock) router.push("/app/inspeksi");
 else {
 // Refresh
 ...
 <Link href="/app/inspeksi" className="text-primary text-sm font-medium hover:underline">← Kembali ke Inspeksi</Link>
 </div>
 );
 ...
 <Link href="/app/inspeksi" className="p-2 hover:bg-surface-hover rounded-xl border border-surface-border glass transition-colors">
 <ArrowLeft size={20} />
 </Link>
 <div>
 ...
 disabled={saving}
 className="px-6 py-2.5 text-sm font-bold bg-primary text-primary-foreground rounded-xl hover: transition-all flex items-center gap-2 disabled:opacity-70"
 >
