...
import type { WODetail, WOReview, WOStage, WOItem, WOPhoto } from "@/types/portal";
...
function ReviewPrompt({ woId, onSubmitted }: { woId: number, onSubmitted: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await portalFetch("/api/v1/customer-auth/review", {
        ...
        body: JSON.stringify({ woId, rating, comment, tags })
      });
      if (res.ok) {
        onSubmitted();
      } else {
...
export default function PortalSpkDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [spk, setSpk] = useState<WODetail | null>(null);
  ...
  const fetchDetail = useCallback(async (showLoader = true) => {
    ...
    try {
      const res = await portalFetch(`/api/v1/customer-auth/wo/${id}`);
      ...
      if (data.success) {
        ...
        setError(data.message || "Gagal memuat Detail Work Order");
      }
    } catch (err) {
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: `Detail Work Order ${spk?.noWo}`, url });
  ...
  if (loading) {
    return (
      <div className="min-h-[50vh] flex flex-col items-center justify-center gap-3">
        ...
        <p className="text-xs text-muted-foreground animate-pulse">Memuat Detail Work Order...</p>
      </div>
    );
  }
  ...
  if (error || !spk) {
    return (
      <div className="min-h-[50vh] flex flex-col items-center justify-center text-center p-4">
        ...
        <p className="text-sm text-muted-foreground mb-6 max-w-sm">{error || "Work Order tidak ditemukan"}</p>
  ...
  return (
    <div className="max-w-sm md:max-w-2xl mx-auto space-y-6 p-4 pb-6 animate-in fade-in duration-500">
      {/* Lightbox */}
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground font-mono mt-0.5">{spk.noWo}</p>
          </div>
        </div>
      ...
      <div className="glass-panel p-5">
        ...
        <div className="w-full bg-background rounded-full h-2.5 mb-6 overflow-hidden">
          <div
            className={`h-full rounded-full transition-all duration-1000 ease-out ${spk.status === "selesai" ? "bg-emerald-500" : "bg-blue-500"} ${!spk.stages?.length && spk.status !== "selesai" && (spk.progress === 0 || !spk.progress) ? "animate-pulse" : ""}`}
            style={{ width: `${spk.status === 'selesai' ? 100 : (spk.progress > 0 ? spk.progress : (spk.status === 'dikerjakan' ? 5 : 5))}%` }}
          />
        </div>
        ...
        {spk.stages && spk.stages.length > 0 ? (
          <div className="space-y-0 relative ml-2">
            ...
            {spk.stages.map((stage: any, index: number) => {
              const isDone = stage.status === "selesai" || spk.status === "selesai";
              const isWip = stage.status === "dikerjakan" && spk.status !== "selesai";
              ...
              return (
                <div key={stage.id} className={`relative pl-8 py-3 ${!isLast ? "" : ""} ${isDone ? "opacity-100" : isWip ? "opacity-100" : "opacity-40"}`}>
                  ...
                  {isDone ? (
                    ...
                    <div className="absolute left-[1px] top-4 w-3.5 h-3.5 bg-blue-500 rounded-full ring-4 ring-blue-500/20 animate-pulse z-10" />
                  ) : (
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <h4 className={`text-sm font-bold ${isDone ? "text-emerald-500" : isWip ? "text-blue-500" : "text-foreground"}`}>
                        {stage.nama}
                      </h4>
                      {isWip && <span className="text-[9px] bg-blue-500/10 text-blue-500 px-1.5 py-0.5 rounded font-bold uppercase animate-pulse">Aktif</span>}
                    </div>
          ...
          <div className="relative pt-2 pb-2">
            <div className="absolute left-[7px] top-6 bottom-4 w-0.5 bg-emerald-500/30" />
            
            {/* Step 1: Diterima */}
            <div className="relative pl-8 py-3 opacity-100">
                <CheckCircle2 size={16} className="absolute left-0 top-3.5 bg-background text-emerald-500 rounded-full z-10" />
                <h4 className="text-sm font-bold text-emerald-500">Work Order Diterima</h4>
            </div>

            {/* Step 2: Selesai Langsung */}
            <div className="relative pl-8 py-3 opacity-100">
                <CheckCircle2 size={16} className="absolute left-0 top-3.5 bg-background text-emerald-500 rounded-full z-10" />
                <div className="flex items-center gap-2">
                    <h4 className="text-sm font-bold text-emerald-500">Pengerjaan Selesai</h4>
                    <span className="text-[9px] bg-emerald-500/10 text-emerald-500 px-1.5 py-0.5 rounded font-bold uppercase">Selesai</span>
                </div>
                <p className="text-xs text-muted-foreground mt-1">Servis telah diselesaikan oleh mekanik secara langsung (tanpa rincian tahapan dari sistem).</p>
            </div>
          </div>
        ) : spk.status === "dikerjakan" ? (
          <div className="relative pt-2 pb-2">
            <div className="absolute left-[7px] top-6 bottom-4 w-0.5 bg-surface-border/50" />
            
            {/* Step 1: Diterima */}
            <div className="relative pl-8 py-3 opacity-100">
                <CheckCircle2 size={16} className="absolute left-0 top-3.5 bg-background text-emerald-500 rounded-full z-10" />
                <h4 className="text-sm font-bold text-emerald-500">Work Order Diterima</h4>
                <p className="text-xs text-muted-foreground mt-1">Sistem telah mencatat permintaan servis ini.</p>
            </div>

            {/* Step 2: Penugasan */}
            <div className="relative pl-8 py-3 opacity-100">
                <CheckCircle2 size={16} className="absolute left-0 top-3.5 bg-background text-emerald-500 rounded-full z-10" />
                <h4 className="text-sm font-bold text-emerald-500">Mekanik Ditugaskan</h4>
            </div>

            {/* Step 3: Dikerjakan (Dynamic) */}
            <div className="relative pl-8 py-3 opacity-100">
                <div className="absolute left-[1px] top-4 w-3.5 h-3.5 bg-blue-500 rounded-full ring-4 ring-blue-500/20 animate-pulse z-10" />
                <div className="flex items-center gap-2">
                    <h4 className="text-sm font-bold text-blue-500">
                        {spk.progress > 0 ? (spk.progress === 100 ? "Proses Perbaikan Fisik Selesai" : "Proses Perbaikan Berjalan") : "Mekanik Menganalisa"}
                    </h4>
                    <span className="text-[9px] bg-blue-500/10 text-blue-500 px-1.5 py-0.5 rounded font-bold uppercase animate-pulse">
                        {spk.progress > 0 ? `${spk.progress}%` : "Aktif"}
                    </span>
                </div>
                <p className="text-xs text-muted-foreground mt-1">
                    Mekanik {spk.mekanik?.name || "yang bertugas"} sedang mengerjakan kendaraan Anda. 
                    {spk.progress === 100 
                        ? " Pengerjaan fisik telah selesai (100%), sistem sedang menunggu mekanik melakukan finalisasi data dan tagihan." 
                        : (spk.progress > 0 
                            ? ` Saat ini progres keseluruhan telah mencapai ${spk.progress}%.` 
                            : " Sedang memeriksa kendaraan untuk tindakan lebih lanjut.")}
                </p>
            </div>
          </div>
        ) : (
          <div className="relative pt-2 pb-2">
            ...
            {/* Step 2: Menunggu Mekanik */}
            <div className="relative pl-8 py-3 opacity-100">
                <div className="absolute left-[1px] top-4 w-3.5 h-3.5 bg-amber-500 rounded-full ring-4 ring-amber-500/20 animate-pulse z-10" />
                <div className="flex items-center gap-2">
                    <h4 className="text-sm font-bold text-amber-500">Menunggu Penugasan Mekanik</h4>
                    <span className="text-[9px] bg-amber-500/10 text-amber-500 px-1.5 py-0.5 rounded font-bold uppercase animate-pulse">Menunggu</span>
                </div>
                <p className="text-xs text-muted-foreground mt-1">Sedang mempersiapkan mekanik untuk menganalisa kendaraan Anda secara langsung.</p>
            </div>
            
            {/* Step 3: Pengerjaan (Dimmed) */}
            <div className="relative pl-8 py-3 opacity-40">
                <Circle size={14} className="absolute left-[1px] top-4 bg-background text-surface-border fill-background z-10" />
                <h4 className="text-sm font-bold text-foreground">Proses Perbaikan</h4>
                <p className="text-xs text-muted-foreground mt-1">Daftar tahapan teknis (seperti bongkar mesin, ganti oli, dll) akan otomatis muncul di sini saat mekanik mulai bekerja.</p>
            </div>
          </div>
        )}
      </div>
      ...
      {spk.pembayaran && (
        <div className="bg-background border border-primary/20 rounded-xl p-5 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="text-center sm:text-left">
            <p className="text-[10px] font-bold text-primary uppercase tracking-wider mb-1">Total Transaksi</p>
            <p className="text-2xl font-black text-primary font-mono">Rp {totalTagihan.toLocaleString("id-ID")}</p>
          </div>
          <Link
            href={`/pub/pembayaran/${spk.pembayaran.publicId}/kwitansi`}
          ...
          >
            <Receipt size={16} /> Lihat Kwitansi
          </Link>
        </div>
      )}
      ...
      {spk.status === "selesai" && !review && (
        <ReviewPrompt woId={spk.id} onSubmitted={() => fetchDetail(false)} />
      )}
      {review && (
      ...
      <div className="text-center pt-4 pb-8">
        ...
        <div className="flex flex-col gap-3">
          <a
            href={`https://wa.me/${process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || "62274123456"}?text=${encodeURIComponent(`Halo MMT Racing, saya ingin bertanya tentang SPK ${spk.noWo}`)}`}
            target="_blank"
