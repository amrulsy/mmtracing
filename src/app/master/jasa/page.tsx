...
export default function JasaPage() {
  ...
  const handleDelete = async (id: number, nama: string) => {
    if (!confirm(`Hapus jasa "${nama}"?`)) return;
    setDeleting(id);
    try {
      ...
      toast.error("Gagal", err.message || "Gagal menghapus jasa");
    } finally {
      setDeleting(null);
    }
  };
  ...
  return (
    <div className="space-y-4 lg:space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <Link href="/master/jasa/tambah" className="flex items-center gap-1.5 btn-glossy bg-primary text-primary-foreground px-3 py-2 rounded-xl text-sm font-medium shadow-glossy-primary hover:shadow-glossy-primary-dark">
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {loading ? (
                    ...
                    <tr><td colSpan={7} className="px-4 py-8 text-center text-muted-foreground">Tidak ada data jasa</td></tr>
                  ) : (
                    services.map((s) => (
                      <tr key={s.id} className="bg-surface border-b border-surface-border hover:bg-surface-hover/30 transition-colors">
                        ...
                        <td className="px-4 py-3 text-right">
                          <div className="flex justify-end gap-1">
                            <Link href={`/master/jasa/tambah?edit=${s.id}`} className="p-1.5 rounded-lg hover:bg-surface-hover text-muted-foreground hover:text-foreground"><Edit size={14} /></Link>
            ...
            {loading ? (
              ...
              <div key={s.id} className="glass-panel p-3 flex items-center gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm font-bold text-primary mt-1 font-mono">{formatRp(s.harga)}</p>
                </div>
                <div className="flex flex-col gap-1 shrink-0">
                  <Link href={`/master/jasa/tambah?edit=${s.id}`} className="p-1.5 rounded-lg hover:bg-surface-hover text-muted-foreground border border-surface-border"><Edit size={14} /></Link>
                  <button onClick={() => handleDelete(s.id, s.name)} disabled={deleting === s.id}
