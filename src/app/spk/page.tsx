    ...
    <div className="space-y-4 lg:space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          <Link href="/spk/create?mode=bubut" className="flex items-center gap-1.5 text-sm bg-surface border border-surface-border px-3 py-2 rounded-xl font-medium hover:bg-surface-hover">
            <Hammer size={16} /> Bubut Lepas
          </Link>
          <Link href="/spk/create" className="flex items-center gap-1.5 btn-glossy bg-primary text-primary-foreground px-3 py-2 rounded-xl text-sm font-medium shadow-glossy-primary hover:shadow-glossy-primary-dark">
              ...
              {loading ? (
                ...
                spks.map((spk) => (
                  <tr key={spk.id} className="bg-surface border-b border-surface-border hover:bg-surface-hover/50 transition-colors">
                    ...
                    <td className="px-6 py-4 font-medium">{formatRp(spk.totalHarga ?? 0)}</td>
                    <td className="px-6 py-4 text-right"><Link href={`/spk/${spk.id}`} className="text-primary hover:text-primary-hover font-medium text-sm">Detail</Link></td>
                  </tr>
                ))
              )}
            </tbody>
      ...
      <div className="lg:hidden space-y-2">
        {loading ? (
          ...
          spks.map((spk) => (
            <Link key={spk.id} href={`/spk/${spk.id}`} className="glass-panel p-4 flex items-center gap-3 active:scale-[0.98] transition-transform block">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm font-bold text-primary mt-1">{formatRp(spk.totalHarga ?? 0)}</p>
              </div>
