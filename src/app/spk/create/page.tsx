...
export default function CreateSpkPage() {
  ...
  // Load pelanggan dan mekanik
  useEffect(() => {
    Promise.all([
      api.getPaginated<Pelanggan>("/pelanggan", { limit: 200 }),
      api.getPaginated<Mekanik>("/mekanik", { limit: 100 }),
    ])
      .then(([pelRes, mekRes]) => {
        setPelangganList(pelRes.data);
        setMekanikList(mekRes.data);
      })
      .catch(() => {})
  ...
  useEffect(() => {
    if (!pelangganId || mode === "bubut") {
      setKendaraanList([]);
      if (!searchParams.get("kendaraanId")) setKendaraanId("");
      return;
    }
    api.get<Pelanggan>(`/pelanggan/${pelangganId}`)
      .then(res => setKendaraanList(res.data.kendaraan || []))
      .catch(() => {});
  }, [pelangganId, mode, searchParams]);

  // Sync display names from URL params
  useEffect(() => {
    if (!loadingOptions && pelangganId && !pelangganSearch) {
       const p = pelangganList.find(x => x.id.toString() === pelangganId);
       if (p) setPelangganSearch(p.name);
    }
  }, [loadingOptions, pelangganList, pelangganId]);
  
  useEffect(() => {
    if (kendaraanList.length > 0 && kendaraanId && !kendaraanSearch) {
       const k = kendaraanList.find(x => x.id.toString() === kendaraanId);
       if (k) setKendaraanSearch(`${k.plat} - ${k.name}`);
    }
  ...
  const handleAddPelanggan = async (e: FormEvent) => {
    ...
    try {
      ...
      toast.error("Gagal", err.message || "Gagal menambah pelanggan");
  ...
  const handleAddKendaraan = async (e: FormEvent) => {
    ...
    try {
      ...
      toast.error("Gagal", err.message || "Gagal menambah kendaraan");
    } finally { setAddingKendaraan(false); }
  };

  const filteredPelanggan = pelangganList.filter(p => p.name.toLowerCase().includes(pelangganSearch.toLowerCase()) || p.phone.includes(pelangganSearch));
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (!pelangganId) { setError("Pelanggan harus dipilih"); return; }
      ...
      <form onSubmit={handleSubmit}>
        <div className="glass-panel overflow-hidden">
              ...
              {mode === "modifikasi" && (
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    ...
                    {stages.map((stage, i) => (
                      <div key={i} className="grid grid-cols-[1fr_120px_80px_auto] gap-2 items-start">
                    ...
                    <div className="text-xs text-muted-foreground">Nama · Estimasi Biaya · Durasi (hari)</div>
          ...
          <div className="p-4 border-t border-surface-border bg-surface-hover/30 flex justify-end gap-3">
            <Link href="/spk" className="px-4 py-2 text-sm font-medium border border-surface-border rounded-xl hover:bg-surface transition-colors">
              Batal
            </Link>
