...
import { Users, Shield, Key, Bell, Palette, Save, Plus, Edit, Trash2, MessageSquare, Database, Loader2, X, Eye, EyeOff, AlertTriangle } from "lucide-react";
import { api } from "@/lib/api";
import { toast } from "@/lib/toast";
import { Skeleton } from "@/components/ui/loading-skeleton";

const settingsTabs = [
  { icon: Users, label: "User", href: "/settings" },
  { icon: Shield, label: "Role Akses", href: "/settings/roles" },
  { icon: Key, label: "Keamanan", href: "/settings/keamanan" },
  { icon: Bell, label: "Notifikasi", href: "/settings/notifikasi" },
  { icon: Palette, label: "Tampilan", href: "/settings/tampilan" },
  { icon: MessageSquare, label: "WhatsApp", href: "/settings/whatsapp" },
  { icon: Database, label: "Backup", href: "/settings/backup" },
];
...
const emptyUser = { name: "", username: "", email: "", password: "", role: "sa" };
...
export default function SettingsPage() {
  ...
  const saveProfile = async () => {
    setSaving(true);
    try {
      await api.put("/settings/config", profile);
  ...
  const openEditUser = (u: User) => {
    setEditingUser(u);
    setUserForm({ name: u.name, username: u.username, email: u.email || "", password: "", role: u.role?.name || "sa" });
    setShowUserModal(true);
  };

  const handleDeleteUser = async (u: User) => {
    if (!window.confirm(`Hapus user "${u.name}"? Tindakan ini tidak dapat dibatalkan.`)) return;
    try {
            ...
            <form onSubmit={handleUserSubmit} className="p-5 space-y-3">
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  <input type={showPw ? "text" : "password"} value={userForm.password} onChange={e => setUserForm(f => ({ ...f, password: e.target.value }))} required={!editingUser} placeholder={editingUser ? "••••••••" : "Minimal 8 karakter"} className="w-full bg-surface border border-surface-border rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 pr-10" />
