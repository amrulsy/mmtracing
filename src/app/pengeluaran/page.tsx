    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex flex-col gap-1">
          <h1 className="text-3xl font-bold tracking-tight">Pengeluaran Operasional</h1>
          <p className="text-muted-foreground">Catat dan pantau biaya operasional bengkel di luar pembelian sparepart.</p>
        </div>
        <button
          onClick={openAdd}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="glass-panel p-4 col-span-2">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs text-muted-foreground font-medium">Total Pengeluaran Bulan Ini</p>
              <p className="text-3xl font-bold text-primary mt-1">
                {summary ? formatRp(summary.bulanIni) : <Skeleton className="h-9 w-32 mt-1" />}
              </p>
            </div>
            {summary && (
              <div className={`flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold ${trendUp ? "bg-red-500/10 text-red-500" : "bg-emerald-500/10 text-emerald-600"}`}>
                {trendUp ? <TrendingUp size={14} /> : <TrendingDown size={14} />}
                {trendStr} vs bulan lalu
              </div>
            )}
          </div>
        </div>
              ...
              return (
                <div key={i}>
                  <div className="flex justify-between text-xs mb-1">
                    ...
                    <span className="text-muted-foreground">{formatRp(amount)} ({percentage}%)</span>
                  </div>
      ...
      <div className="glass-panel overflow-hidden">
        <div className="p-4 border-b border-surface-border bg-surface-hover/30 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        {error ? (
          ...
          <div className="divide-y divide-surface-border">
            {loading ? (
              ...
              expenses.map((exp) => (
                <div key={exp.id} className="flex items-center gap-4 px-5 py-3.5 hover:bg-surface-hover/20 transition-colors">
                  ...
                  <span className="text-sm font-bold text-red-500 font-mono shrink-0">- {formatRp(exp.jumlah)}</span>
