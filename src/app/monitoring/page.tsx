...
import { Filter, Camera, MessageSquare, AlertCircle, Loader2, RefreshCw } from "lucide-react";
...
export default function MonitoringPage() {
  ...
  const renderCard = (card: Spk) => (
    <Link key={card.id} href={`/spk/${card.id}`} className={`glass p-4 rounded-xl cursor-pointer hover:-translate-y-1 hover:shadow-glossy transition-all block ${card.prioritas === "urgent" || card.prioritas === "tinggi" ? "border-l-4 border-l-amber-500" : ""}`}>
